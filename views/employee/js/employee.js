...
var openUserFormModal = function(user = false) {
    ...
    var title = user ? "<i class='iconsminds-file-edit with-rotate-icon'></i> Edit Payable" :
        "<i class='iconsminds-add-user'></i> Create Employee";
...
var setEdituserFormValues = function(user) {
    ...
    $.each(user, function(key, value) {
        ...
        if (key == "EmployeeStatus_idEmployeeStatus") {

            $("#EmployeeStatus_idEmployeeStatus option[value='" + value + "']").attr("selected", true).trigger('change');
        }

        if (key == "DocumentType_idDocumentType") {

            $("#DocumentType_idDocumentType option[value='" + value + "']").attr("selected", true).trigger('change');
...
var loadUsers = async function() {
    let Users = await webClient.RequestAsync("employee/getAllEmployee", "", webClient.ContentType.DEFAULT);
...
var loadDocument = async function() {
    let Documents = await webClient.RequestAsync("employee/getAllEmployeeDocument", "", webClient.ContentType.DEFAULT);
    if (Documents.status === REQUEST_STATUS.ERROR) return;
    if (typeof Documents.data === "undefined" || Documents.data === '' || Documents.data.length === 0) return;

    showDocuments(Documents.data);

};

var loadStatus = async function() {
    let Status = await webClient.RequestAsync("employee/getAllEmployeeStatus", "", webClient.ContentType.DEFAULT);
...
var showDocuments = function(Documents) {
    ...
    $.each(Documents, function(index, document) {
        ...
        var drow = JSON.stringify(document);


        $('#DocumentType_idDocumentType').append('<option value= "' + document.idDocumentType + '">' + document.DocumentTypeDescription + '</option>');
...
var showStatus = function(Status) {
    ...
    $.each(Status, function(index, state) {
        ...
        $('#EmployeeStatus_idEmployeeStatus').append('<option value= "' + state.idEmployeeStatus + '">' + state.EmployeeStatusDescription + '</option>');
...
var showUsers = function(Users) {
    ...
    $.each(Users, function(index, user) {
        UsersHtml += "<div class='card mb-3 user-list-thumb'>" +
            ...
            "<div class='d-flex p-4'>" +
            "<a class='edit-user-link d-flex align-items-center' data-user='" + JSON.stringify(user) + "' href=''>" +
            "Edit Payable<h2 class='m-0'><i class='iconsminds-file-edit with-rotate-icon'></i></h2>" +
            "</a>"
            /*  + "<button class='btn btn-outline-theme-3 icon-button rotate-icon-click collapsed align-self-center'"
              + " type='button' data-toggle='collapse' data-target='#company-data-collapse-container-" + index 
              + "' aria-expanded='false' aria-controls='q2'>"
...
var getUserBasicDataHtml = function(user) {
    ...
    var basicDataHtml = "<div class='d-flex flex-grow-1 min-width-zero'>" +
        ...
        "<p class='list-item-heading mb-0 truncate'>" + user.EmployeName + " " + user.EmployeeLastName + "</p>" +

        "<p class='mb-0 text-muted w-15 w-sm-100'>" +
        "<span class='glyph-icon iconsminds-id-card align-text-top' style='font-size: 25px;'></span> " +
        "<span class='align-middle'>" + user.EmployeePhoneNumber + "</span>" +
        "</p>" +
...
var getUserCollapsedDataHtml = function(index, user) {
    ...
    var userCollapsedData = "<div class='collapse p-2 border-top' id='user-data-collapse-container-" + index + "'>" +
        ...
        "<a class='edit-user-link d-flex align-items-center' data-user='" + JSON.stringify(user) + "' href=''>" +
        "Edit Employee<h2 class='m-0'><i class='iconsminds-file-edit with-rotate-icon'></i></h2>" +
        "</a>" +
        "</div>" +
        "</div>" +
        "<div class='card'>" +
...
var createUser = async function() {
    ...
    if (typeof cropper !== "undefined")
        userFormData += "&basicDataPhoto=" + cropper.getCroppedCanvas().toDataURL();


    var result = await webClient.RequestAsync("employee/CreateEmployee", userFormData, webClient.ContentType.DEFAULT);
